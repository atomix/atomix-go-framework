...
func RegisterProxy(node *passthrough.Node) {
	node.RegisterProxy("Map", func(server *grpc.Server, client *passthrough.Client) {
...
func (s *Proxy) Restore(srv _map.MapService_RestoreServer) error {
	var streams map[passthrough.PartitionID]_map.MapService_RestoreClient
	for {
		...
		if streams == nil {
			partitions := s.Partitions()
			streams = make(map[passthrough.PartitionID]_map.MapService_RestoreClient)
