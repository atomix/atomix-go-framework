...
func (s *ServiceAdaptor) Restore(reader io.Reader) error {
	for {
		...
		entry := &indexedmap.SnapshotEntry{}
...
func (s *ServiceAdaptor) prevEntry(in []byte) ([]byte, error) {
	input := &indexedmap.PrevEntryInput{}
	err := proto.Unmarshal(in, input)
	if err != nil {
		s.log.Error(err)
		return nil, err
...
func (s *ServiceAdaptor) nextEntry(in []byte) ([]byte, error) {
	input := &indexedmap.NextEntryInput{}
	err := proto.Unmarshal(in, input)
	if err != nil {
		s.log.Error(err)
		return nil, err
...
func (s *ServiceAdaptor) entries(in []byte, stream rsm.Stream) (rsm.StreamCloser, error) {
	input := &indexedmap.EntriesInput{}
	err := proto.Unmarshal(in, input)
	if err != nil {
		s.log.Error(err)
		return nil, err
	...
	closer, err := s.rsm.Entries(input, output)
	if err != nil {
		s.log.Error(err)
		return nil, err
	}
...
func (s *ServiceAdaptor) restore(in []byte) ([]byte, error) {
	input := &indexedmap.SnapshotEntry{}
	err := proto.Unmarshal(in, input)
	if err != nil {
		s.log.Error(err)
		return nil, err
