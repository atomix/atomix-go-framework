...
func RegisterPassthroughProxy(node *passthrough.Node) {
	node.RegisterProxy(Type, func(server *grpc.Server, client *passthrough.Client) {
		api.RegisterIndexedMapServiceServer(server, &PassthroughProxy{
...
func (s *PassthroughProxy) Events(request *api.EventRequest, srv api.IndexedMapService_EventsServer) error {
...
func (s *PassthroughProxy) Entries(request *api.EntriesRequest, srv api.IndexedMapService_EntriesServer) error {
