...
func (s *ServiceAdaptor) ExecuteCommand(command rsm.Command) {
	...
	case 2:
		...
		log.Debugf("Proposal AppendProposal %.250s", p)
		response, err := s.rsm.Append(p)
		if err != nil {
			log.Debugf("Proposal AppendProposal %.250s failed: %v", p, err)
			command.Output(nil, err)
		} else {
			output, err := proto.Marshal(response)
			if err != nil {
				err = errors.NewInternal(err.Error())
				log.Errorf("Proposal AppendProposal %.250s failed: %v", p, err)
				command.Output(nil, err)
			} else {
				log.Debugf("Proposal AppendProposal %.250s complete: %.250s", p, response)
	...
	case 3:
		...
		log.Debugf("Proposal InsertProposal %.250s", p)
		response, err := s.rsm.Insert(p)
		if err != nil {
			log.Debugf("Proposal InsertProposal %.250s failed: %v", p, err)
			command.Output(nil, err)
		} else {
			output, err := proto.Marshal(response)
			if err != nil {
				err = errors.NewInternal(err.Error())
				log.Errorf("Proposal InsertProposal %.250s failed: %v", p, err)
				command.Output(nil, err)
			} else {
				log.Debugf("Proposal InsertProposal %.250s complete: %.250s", p, response)
				command.Output(output, nil)
			}
		}
		command.Close()
