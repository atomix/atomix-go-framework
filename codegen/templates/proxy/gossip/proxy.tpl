...
func (s *{{ $proxy }}) {{ .Name }}(srv {{ template "type" $primitive.Type }}_{{ .Name }}Server) error {
    ...
    {{- else if .Scope.IsGlobal }}
    var streams map[gossip.PartitionID]{{ template "type" $primitive.Type }}_{{ .Name }}Client
    {{- end }}
    for {
        ...
        if streams == nil {
            partitions := s.Partitions()
            streams = make(map[gossip.PartitionID]{{ template "type" $primitive.Type }}_{{ .Name }}Client)
