...
func (s *{{ $proxy }}) {{ .Name }}(srv {{ template "type" $primitive.Type }}_{{ .Name }}Server) error {
    ...
    {{- else if .Scope.IsGlobal }}
    var streams map[passthrough.PartitionID]{{ template "type" $primitive.Type }}_{{ .Name }}Client
    {{- end }}
    for {
        ...
        if stream == nil {
            ...
            client := {{ $primitive.Type.Package.Alias }}.New{{ $primitive.Type.Name }}Client(conn)
            stream, err = client.{{ .Name }}(srv.Context())
            if err != nil {
                return err
            }
        }
        ...
        if streams == nil {
            partitions := s.Partitions()
            streams = make(map[passthrough.PartitionID]{{ template "type" $primitive.Type }}_{{ .Name }}Client)
